    ...
    <div id="pontuacao" class="tab-pane active">
        ...
        <div width="100%">
            <table id="tabela" border="0" width="100%" style="text-align: left">
                ...
                $sql = "SELECT *, SUM(pt.total_de_pontos) AS total FROM (militar_estadual AS me JOIN pontos_acumulados AS pt ON me.id_militar_estadual = pt.id_militar_estadual) WHERE pt.mes_vigencia = {$mes} AND pt.ativo = 1 GROUP BY pt.mes_vigencia, me.subunidade, me.unidade ORDER BY me.nome ASC";
                $sql = $pdo->query($sql);
                if ($sql->rowCount() > 0):
                    ?>
                    <tr>
